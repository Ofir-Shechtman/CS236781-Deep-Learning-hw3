...
class EncoderCNN(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        for channel_in, channel_out in [(in_channels, 64), (64, 128), (128, out_channels)]:
            ...
            modules.append(BatchNorm2d(num_features=channel_out))
            modules.append(ReLU())
...
class Unflatten(nn.Module):
    ...
    def forward(self, input: Tensor) -> Tensor:
        return input.reshape(len(input), *self.unflattened_size)
...
class VAE(nn.Module):
    def __init__(self, features_encoder, features_decoder, in_size, z_dim):
        ...
        self.eval()
        self.add_module(f'encoder_fc', self.encoder_fc)
        self.add_module(f'l_mu', self.l_mu)
        self.add_module(f'l_var', self.l_var)
        self.add_module(f'decoder_fc', self.decoder_fc)
    ...
    def encode(self, x):
        ...
        encoded_flat = torch.flatten(encode, start_dim=1)
        h = self.encoder_fc(encoded_flat)
        h = nn.ReLU(True)(h)
        mu = self.l_mu(h)
        logvar = self.l_var(h)
        ...
        z = mu + (torch.exp(logvar) ** 0.5) * sample
