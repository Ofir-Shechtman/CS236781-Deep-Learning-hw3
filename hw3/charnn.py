...
def generate_from_model(model, start_sequence, n_chars, char_maps, T):
    ...
    with torch.no_grad():
        ...
        last_layer, hidden_state = model(sample.to(dtype=torch.float, device=device), hidden_state=None)
        ...
        for i in range(n_chars - len(start_sequence)):
            ...
            sample = sample.unsqueeze(0)
            last_layer, hidden_state = model(sample.to(dtype=torch.float, device=device), hidden_state)
    # ========================
...
class GRULayer(nn.Module):
    ...
    def forward(self, input: Tensor, hidden_state: Tensor):
        ...
        for char in range(seq_len):
            z = torch.sigmoid(self.Linear_Wxz(layer_input[:, char, :]) + self.Linear_Whz(h))
            r = torch.sigmoid(self.Linear_Wxr(layer_input[:, char, :]) + self.Linear_Whr(h))
            g = torch.tanh(self.Linear_Wxg(layer_input[:, char, :]) + self.Linear_Whg(r * h))
            h = z * h + (1 - z) * g
            if self.dropout:
                layer_states.append(self.Dropout(h))
            else:
                layer_states.append(h)
            
        return torch.stack(layer_states, dim=1), h
...
class MultilayerGRU(nn.Module):
    ...
    def forward(self, input: Tensor, hidden_state: Tensor = None):
        ...
        for i, layer in enumerate(self.layers):
            layer_input, h = layer(layer_input, layer_states[i])
            final_h.append(h)
        ...
        return torch.stack(l, dim=0), torch.stack(final_h, dim=1)
